...
export const hubMemberCandidatesAtom = atom<Promise<Array<Profile>>>(
    async (get) => {
        const hub = get(currentHubAtom);
        return hub
            ? getUserCandidates(get(dbClientAtom), hub.id)
            : new Array<Profile>();
    }
);

export const hubProjectCandidatesAtom = atom<Promise<Array<Project>>>(
    async (get) => {
        const hub = get(currentHubAtom);
        return hub
            ? getProjectCandidatesForHub(get(dbClientAtom), hub.id)
            : new Array<Project>();
    }
);

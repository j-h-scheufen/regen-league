...
export default function PasswordForm({mode, onSuccess, onCancel}: Props) {
    ...
    const [oldPasswordValidated, setOldPasswordValidated] = useAtom(oldPasswordValidatedAtom)
    ...
    const setUserPassword = useCallback( async (): Promise<User> => {
        try {
            ...
            const updateResult = await client.auth.updateUser({password: newPassword.pwd1})
    ...
    const validateNewPassword = () => {
        ...
        return newPassword.pwd1.length >= 8 &&
            newPassword.pwd1 === newPassword.pwd2
    }
        ...
        <Box pad="small">
            <Form<NewPassword>
                ...
                <FormField name="pwd1" htmlFor="pwd1Id" label="New Password">
                    <TextInput id="pwd1Id" name="pwd1" type="password" placeholder="Ul3$%_5Ktss75"/>
                </FormField>
                <FormField name="pwd2" htmlFor="pwd2Id" label="Repeat Password">
                    <TextInput id="pwd2Id" name="pwd2" type="password" placeholder="Ul3$%_5Ktss75"/>
                </FormField>
                <Box direction="row" gap="medium" margin={{top: "medium"}} justify="end">
                    {mode != Mode.REGISTER &&

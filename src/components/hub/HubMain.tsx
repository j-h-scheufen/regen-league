...
export default function HubMain() {
  const [hubRoles, initialHubCandidates] = useAtomValue(
    waitForAll([hubRolesAtom, hubMemberCandidatesAtom])
  );
  ...
  if (!currentHub) throw Error("No hub object detected in app state!");
      ...
      <Box width="large">
        ...
        {edit ? (
          <Box>
            ...
            />
            <HubAttributesForm hub={currentHub} />
            <LocationForm entity={currentHub} update={updateLocation} />
            ...
            />
            <ProjectConnectionsForm hubId={currentHub.id} />
          </Box>
        ) : (
          <Box>
            {isAdmin && (

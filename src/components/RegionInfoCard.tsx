  ...
  return (
    <Card pad="small">
      ...
      <CardBody margin={{ top: "xxsmall" }}>
        {region && (
          <Text>
            <LinkWrapper condition={region.link} url={region.link}>
              {region.name}
            </LinkWrapper>
          </Text>
        )}
            ...
            <Box>
              {associations?.oneEarth && associations.oneEarth.length > 0 && (
                <Box direction="column">
                  ...
                  <Box direction="row" width="100%" gap="small">
                    {oneEarthLabels.map((label: string, index) => {
                      if (
                        associations?.oneEarth &&
                        associations.oneEarth[index]
                      )
                        return (
                          <RegionCard
                            key={index}
                            region={associations.oneEarth[index]}
                            title={label}
                          />
                        );
                    })}
              ...
              {associations?.epa && associations.epa.length > 0 && (
                <Box direction="column">
                  ...
                  <Box direction="row">
                    <Box direction="row" width="100%" gap="small">
                      {epaLabels.map((label: string, index) => {
                        if (associations?.epa && associations.epa[index])
                          return (
                            <RegionCard
                              key={index}
                              region={associations.epa[index]}
                              title={label}
                            />
                          );
                      })}
              ...
              {associations?.custom && associations.custom.length > 0 && (
                <Box direction="column">
                  ...
                  <Box basis="1/2">
                    <Text>
                      {associations.custom[0] && (
                        <Link href={associations.custom[0]?.link || ""}>
                          {associations.custom[0]?.name}
                        </Link>
                      )}
                    </Text>
                  </Box>

...
export default function RegionSelectorPanel({ ownerId }: Props) {
  const [oeCatalog, epaCatalog, customCatalog] = useAtomValue(
    waitForAll([oneEarthCatalogAtom, epaCatalogAtom, customCatalogAtom])
  );
  ...
  const updateRegions = useCallback(async () => {
    ...
    if (oeSelection) {
      if (oeSelection.length > 0) {
        // the length of the array returned from the RegionInfoSelector reflects how many levels down were configured by the user
        const region = oeSelection[oeSelection.length - 1];
        if (region?.id && region.level) oeRegion = { ...region };
      } else oeRegion = null;
    }
    if (epaSelection) {
      if (epaSelection.length > 0) {
        const region = epaSelection[epaSelection.length - 1];
        if (region?.id && region.level) epaRegion = { ...region };
      } else epaRegion = null;
    }
    ...
    if (customSelection) {
      if (customSelection.length > 0) {
        ...
        if (region?.id && region.level) customRegion = { ...region };
      } else customRegion = null;
    }

    const newRegions = await updateRegionAssociations(
      client,
      ownerId!,
      oeRegion,
      epaRegion,
  ...
  return (
    <Card pad="small" margin={{ vertical: "small" }}>
      <CardHeader justify="center">
        <Text size="large">Region Settings</Text>
      </CardHeader>
      <CardBody>
        <Box pad="small">
          <Form
          ...
          >
            <JotaiProvider
              initialValues={
                [[selectionAtom, associations?.oneEarth || null]] as const
              }
            >
              <RegionInfoSelector
                title="One Earth"
            ...
            <JotaiProvider
              initialValues={
                [[selectionAtom, associations?.epa || null]] as const
              }
            >
              <RegionInfoSelector
                title="EPA"
            ...
            <JotaiProvider
              initialValues={
                [[selectionAtom, associations?.custom || null]] as const
              }
            >
              <RegionInfoSelector
                title="Other"
